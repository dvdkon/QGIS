...
class VectorStream
{
  public:
    size_t mOffset = 0;
    VectorStream( const std::vector<char> &vec ) : mVec( vec ) {}
    const void *read( size_t bytes )
    {
      if ( mVec.size() < mOffset + bytes )
      {
        throw QgsException( "Tried to read beyond EOF when parsing quantized mesh tile" );
      }
...
QgsQuantizedMeshTile::QgsQuantizedMeshTile( const std::vector<char> &data )
{
...
tinygltf::Model QgsQuantizedMeshTile::toGltf()
{
  ...
  {
    ( ( float * ) vertexBuffer.data.data() )[i] = mVertexCoords[i];
    if ( coordMinimums[i % 3] > mVertexCoords[i] )
      coordMinimums[i % 3] = mVertexCoords[i];
    if ( coordMaximums[i % 3] < mVertexCoords[i] )
      coordMaximums[i % 3] = mVertexCoords[i];
  }
